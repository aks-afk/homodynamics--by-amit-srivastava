import {
  ...
  wiggle_default,
  wye_default
} from "./chunk-ODIZV37V.js";
import {
  active_default,
  interrupt_default,
  transition
} from "./chunk-4O3T27ZQ.js";
import {
  backIn,
  backInOut,
  backOut,
...
import {
  ...
  timer,
  timerFlush
} from "./chunk-XGS3PSW2.js";
import {
  drag_default,
  nodrag_default,
  yesdrag
} from "./chunk-KQ4UKN7P.js";
import {
  create_default,
  creator_default,
  local,
...
import {
  dispatch_default
} from "./chunk-6WKSR2VE.js";
import {
  Path,
  path,
  pathRound
} from "./chunk-KQIQP2PI.js";
import {
  FormatSpecifier,
  band,
  defaultLocale,
  ...
  *neighbors(i) {
  ...
  _clipSegment(x06, y06, x12, y12, c0, c1) {
    ...
    while (true) {
      ...
      let x, y, c4 = c0 || c1;
      if (c4 & 8) x = x06 + (x12 - x06) * (this.ymax - y06) / (y12 - y06), y = this.ymax;
      else if (c4 & 4) x = x06 + (x12 - x06) * (this.ymin - y06) / (y12 - y06), y = this.ymin;
      else if (c4 & 2) y = y06 + (y12 - y06) * (this.xmax - x06) / (x12 - x06), x = this.xmax;
  ...
  _project(x06, y06, vx, vy) {
    let t = Infinity, c4, x, y;
    if (vy < 0) {
      if (y06 <= this.ymin) return null;
      if ((c4 = (this.ymin - y06) / vy) < t) y = this.ymin, x = x06 + (t = c4) * vx;
    } else if (vy > 0) {
      if (y06 >= this.ymax) return null;
      if ((c4 = (this.ymax - y06) / vy) < t) y = this.ymax, x = x06 + (t = c4) * vx;
    }
    if (vx > 0) {
      if (x06 >= this.xmax) return null;
      if ((c4 = (this.xmax - x06) / vx) < t) x = this.xmax, y = y06 + (t = c4) * vy;
    } else if (vx < 0) {
      if (x06 <= this.xmin) return null;
      if ((c4 = (this.xmin - x06) / vx) < t) x = this.xmin, y = y06 + (t = c4) * vy;
    }
    return [x, y];
  }
  _edgecode(x, y) {
...
var Delaunay = class _Delaunay {
  ...
  *neighbors(i) {
  ...
  find(x, y, i = 0) {
    ...
    const i0 = i;
    let c4;
    while ((c4 = this._step(i, x, y)) >= 0 && c4 !== i && c4 !== i0) i = c4;
    return c4;
  }
  _step(i, x, y) {
  ...
  function clipLine(stream) {
    ...
    return {
      ...
      point: function(lambda, phi2) {
        var point1 = [lambda, phi2], point2, v2 = visible(lambda, phi2), c4 = smallRadius ? v2 ? 0 : code(lambda, phi2) : v2 ? code(lambda + (lambda < 0 ? pi2 : -pi2), phi2) : 0;
...
function resample(project, delta2) {
  function resampleLineTo(x06, y06, lambda04, a0, b0, c0, x12, y12, lambda12, a1, b1, c1, depth, stream) {
    ...
    if (d2 > 4 * delta2 && depth--) {
      var a2 = a0 + a1, b = b0 + b1, c4 = c0 + c1, m2 = sqrt2(a2 * a2 + b * b + c4 * c4), phi2 = asin(c4 /= m2), lambda22 = abs3(abs3(c4) - 1) < epsilon5 || abs3(lambda04 - lambda12) < epsilon5 ? (lambda04 + lambda12) / 2 : atan2(b, a2), p = project(lambda22, phi2), x2 = p[0], y2 = p[1], dx2 = x2 - x06, dy2 = y2 - y06, dz = dy * dx2 - dx * dy2;
      if (dz * dz / d2 > delta2 || abs3((dx * dx2 + dy * dy2) / d2 - 0.5) > 0.3 || a0 * a1 + b0 * b1 + c0 * c1 < cosMinDistance) {
        resampleLineTo(x06, y06, lambda04, a0, b0, c0, x2, y2, lambda22, a2 /= m2, b /= m2, c4, depth, stream);
        stream.point(x2, y2);
        resampleLineTo(x2, y2, lambda22, a2, b, c4, x12, y12, lambda12, a1, b1, c1, depth, stream);
      }
    }
  }
  return function(stream) {
...
function conicEqualAreaRaw(y06, y12) {
  ...
  project.invert = function(x, y) {
    ...
    return [l / n, asin((c4 - (x * x + r0y * r0y) * n * n) / (2 * n))];
  };
  return project;
}
...
function azimuthalInvert(angle2) {
  return function(x, y) {
    var z = sqrt2(x * x + y * y), c4 = angle2(z), sc = sin2(c4), cc2 = cos2(c4);
    return [
      atan2(x * sc, z * cc2),
      asin(z && y * sc / z)
    ];
...
function conicConformalRaw(y06, y12) {
  ...
  function project(x, y) {
    if (f > 0) {
      if (y < -halfPi2 + epsilon5) y = -halfPi2 + epsilon5;
    } else {
      if (y > halfPi2 - epsilon5) y = halfPi2 - epsilon5;
    }
...
function encloseBasis3(a2, b, c4) {
  var x12 = a2.x, y12 = a2.y, r1 = a2.r, x2 = b.x, y2 = b.y, r2 = b.r, x3 = c4.x, y3 = c4.y, r3 = c4.r, a22 = x12 - x2, a3 = x12 - x3, b2 = y12 - y2, b3 = y12 - y3, c22 = r2 - r1, c32 = r3 - r1, d1 = x12 * x12 + y12 * y12 - r1 * r1, d2 = d1 - x2 * x2 - y2 * y2 + r2 * r2, d3 = d1 - x3 * x3 - y3 * y3 + r3 * r3, ab4 = a3 * b2 - a22 * b3, xa = (b2 * d3 - b3 * d2) / (ab4 * 2) - x12, xb = (b3 * c22 - b2 * c32) / ab4, ya = (a3 * d2 - a22 * d3) / (ab4 * 2) - y12, yb = (a22 * c32 - a3 * c22) / ab4, A = xb * xb + yb * yb - 1, B2 = 2 * (r1 + xa * xb + ya * yb), C = xa * xa + ya * ya - r1 * r1, r = -(Math.abs(A) > 1e-6 ? (B2 + Math.sqrt(B2 * B2 - 4 * A * C)) / (2 * A) : C / B2);
  return {
    x: x12 + xa + xb * r,
    y: y12 + ya + yb * r,
    r
...
function packSiblingsRandom(circles, random) {
  ...
  var a2, b, c4, n, aa2, ca3, i, j, k, sj, sk;
...
function centroid_default3(polygon) {
  var i = -1, n = polygon.length, x = 0, y = 0, a2, b = polygon[n - 1], c4, k = 0;
  while (++i < n) {
    a2 = b;
    b = polygon[i];
...
function contains_default3(polygon, point2) {
  ...
  for (var i = 0; i < n; ++i) {
    ...
    if (y12 > y !== y06 > y && x < (x06 - x12) * (y - y12) / (y06 - y12) + x12) inside = !inside;
    x06 = x12, y06 = y12;
  }
  return inside;
}
