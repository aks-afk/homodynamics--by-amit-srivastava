...
import { visualizationRegistry, getVisualizationConfig } from '../utils/visualizationRegistry';
...
const VisualizationError: React.FC<{ error?: Error; retry?: () => void; componentName?: string }> = ({ 
  error, 
  retry, 
  componentName 
}) => (
...
const importVisualization = async (componentName: string): Promise<ComponentType<any>> => {
  if (componentCache.has(componentName)) {
    return componentCache.get(componentName)!;
  }

  const config = getVisualizationConfig(componentName);
  if (!config) {
    throw new Error(`Visualization component "${componentName}" not found in registry`);
  }
  ...
  try {
    ...
    if (!Component) {
      throw new Error(`No default export found in ${config.importPath}`);
    }
    
    componentCache.set(componentName, Component);
    return Component;
  } catch (error) {
    console.error(`Failed to load visualization: ${componentName}`, error);
    throw error;
  }
};
...
const LazyVisualizationWrapper: React.FC<LazyVisualizationWrapperProps> = ({ 
  componentName, 
  ...
  const LazyComponent = useMemo(() => {
    return lazy(() => importVisualization(componentName));
  }, [componentName]);
